# IdeaBrowser Idea Builder — "Product Requirements Doc" template prompt (scaffold, verbatim)

Captured 2026-07-18 from a live logged-in session (free tier), same idea as
the landing-page prompt. Generated at
`/hub/ideas/<slug>/build/product-requirements-doc`.

Total length: 25,756 characters = this 9,763-char scaffold + the same
`## BUSINESS CONTEXT` block as every other template for the idea (byte-identical,
15,993 chars — see `idea-builder-landing-page-prompt.md` for that block in full;
it is omitted here to avoid duplicating it in the repo).
...
**1. Executive Summary**
Product: Pre-built AI connectors for franchise chains
...
## BUSINESS CONTEXT
[... identical 15,993-char block — see idea-builder-landing-page-prompt.md ...]
```
